...
from settings import long_string, russian, chinese, special_symb, login_pass, login_email
...
def break_auth_key():
    auth_key_broken = PetFriends().get_api_key(login_email, login_pass)
    return auth_key_broken['key'] + 'r'
...
class TestPostPets():
    ...
    @parametrize("age",
                 ['1'],
                 ids=['just one'])
    def test_new_pet_NO_photo(self, auth_key, name, pet_type, age):
        _, status, result = self.pf.create_simple_pet(auth_key, name, pet_type, age)
    ...
    def test_post_new_pet(self, auth_key, name, pet_type, age, pet_photo):
        pet_photo = os.path.join(os.path.dirname(__file__), pet_photo)

        _, status, result = self.pf.post_newPet(auth_key, name, pet_type, age, pet_photo)
        assert status == 200
        assert result['name'] == name
    ...
    @pytest.mark.ui
    @pytest.mark.event
    def test_added_photo(self, auth_key, pet_photo='images\kenar-vitek.jpg'):
        pet_photo = os.path.join(os.path.dirname(__file__), pet_photo)
